...
6. 根据用户一次回复原子更新事实；不得重复询问已确认事项，一次回复最多调用一次存储工具。
...
9. 按 [references/contracts.md](references/contracts.md) 生成最终方案，展示 revision 和验证 evidence checksum，输出 `PENDING_CONFIRMATION` 等待用户核验。
10. 用户确认当前版本后，生成下一 revision 的 READY RequirementRecord 和下一 version 的 READY_FOR_PLANNING TaskSpec，记录最终核验信息，再调用 `store_task_spec`。
...
- `store_task_spec`：仅在用户允许保存当前任务状态或确认最终方案后使用；必须携带幂等键及 expected revision/version。
