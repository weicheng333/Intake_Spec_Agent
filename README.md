...
明确默认在宿主允许时使用普通文字对话，二至五个独立问题分组；仅存在依赖、用户逐题要求、只剩一题或宿主限制时使用单题。停用弹窗不代表不要选项或推荐说明。弹窗未提交不作为答案，部分回答不自动采用其余推荐项。

宿主禁止文字多选时，Skill不能覆盖该限制：须解释原因，改用允许的开放问题或用户接受的交互方式。静态规则检查不能证明弹窗稳定，也不能证明当前会话可以呈现文字选项。本修复不更改数据库、契约或Executor权限。
